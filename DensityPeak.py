...
class DensityPeak:
    ...
    def get_local_density(self):
        ...
        density_list = np.zeros(X_num, dtype=int)
    ...
    def get_min_distance_list(self):
        ...
        min_distance_list = np.zeros(density_list.__len__())
        for i in range(density_list.__len__()):
            greater_index = np.where(density_list > density_list[i])[0]
            if len(greater_index) == 0:
                min_distance_list[i] = np.max(distance_matrix)
                continue
            b = distance_matrix[i][greater_index]
            min_distance_list[i] = np.min(b)
    ...
    def point_assign(self):
        ...
        for i, index in enumerate(sort_density_index):
            # 按照密度高->低的顺序对每个点预测所属于的类别
            # 从密度比该点大的点集合中寻找距离最小的点。
            if y_predict[index] == 0:
                b = distance_matrix[index][sort_density_index[:i]]
                if b.__len__() == 0:
                    b = distance_matrix[index][outliers_index_list]
                l = np.where(distance_matrix[index] == np.min(b))[0][0]
                y_predict[index] = y_predict[l]
        return y_predict
